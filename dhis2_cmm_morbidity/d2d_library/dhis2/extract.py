...
class DataElementsExtractor:
    ...
    def _retrieve_data(
        self, data_elements: list[str], org_units: list[str], period: str, **kwargs: any
    ) -> pd.DataFrame:
...
class IndicatorsExtractor:
    ...
    def _retrieve_data(self, indicators: list[str], org_units: list[str], period: str, **kwargs: any) -> pd.DataFrame:
        if not self.extractor._valid_dhis2_period_format(period):
            raise ValueError(f"Invalid DHIS2 period format: {period}")
...
class ReportingRatesExtractor:
    ...
    def _retrieve_data(
        self, reporting_rates: list[str], org_units: list[str], period: str, **kwargs: any
    ) -> pd.DataFrame:
        if not self.extractor._valid_dhis2_period_format(period):
            raise ValueError(f"Invalid DHIS2 period format: {period}")
        include_cocs = kwargs.get("include_cocs", False)
...
class DHIS2Extractor:
    ...
    def _map_to_dhis2_format(
        ...
        map_cocs : bool, optional
            Whether to include `categoryOptionCombo` when mapping indicators. Default is False.
